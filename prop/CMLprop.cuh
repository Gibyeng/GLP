...
template<typename V, typename E>
void CMLprop<V, E>::postprocess()
{
    this->free_gmem();
}

template<typename V, typename E>
void CMLprop<V, E>::init_gmem(V n,E m)
{
...
template<typename V, typename E>
int CMLprop<V, E>::binary_search(E left, E right,std::vector<V> offsets, std::vector<V> vertices, V target)
{
	E ans = 0;
...
template<typename V, typename E>
void CMLprop<V, E>::perform_lp(V n,E m,int niter)
{
	//init flag as false
	bool flag = false;
...
//	printf("\n");
	...
	if(num_huge>0){
		...
		// count-min sketch for huge nodes computing
	...
	// first huge(node[0]) -> 0
	cudaMalloc(&d_max_count_huge, sizeof(int)*num_huge);
	cudaMemset(d_max_count_huge, 0, sizeof(int)*num_huge);
...
//    		gather_labels<int,int><<<m_blocks, nthreads, 0, 0>>>(d_neighbors, d_labels, d_adj_labels, m);
    	...
    	if(!opt_load||j<changeiter){
			...
			cudaDeviceSynchronize();
	        l_huge_update_syn<32,nt_huge><<<nb_huge, nt_huge, 0,0>>>( d_neighbors,d_offsets, d_adj_labels,d_labels,d_vertices,d_block_v,d_block_id,d_max_count_huge, d_counter,d_cm);
			l_big_update_syn<32,VT, nt_big,200><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter);
...
template<typename V, typename E>
void CMLprop<V, E>::run(int niter)
{
	this->preprocess();
...
template<typename V, typename E>
void CMLprop<V, E>::testerror(int niter){
