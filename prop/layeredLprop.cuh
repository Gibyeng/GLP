...
public:
    ...
    void assign_seeds_for_device (std::string seedfile, int n);
    GraphT* G;
protected:
    void preprocess();
    void postprocess();
...
template<typename V, typename E>
void layeredLprop<V,E>::preprocess(){
...
template<typename V, typename E>
void layeredLprop<V, E>::postprocess()
{
    this->free_gmem();
}

template<class V, class E>
void layeredLprop<V, E>::errorCheck(std::string message){
...
template<typename V, typename E>
void layeredLprop<V, E>::init_gmem(V n,E m)
{
...
//
    ...
    cudaMalloc(&d_seednodes,sizeof(bool)* n);
}

template<typename V, typename E>
void layeredLprop<V, E>::free_gmem()
{
    ...
    cudaFree(d_seednodes);
}

// Return the number of labels updated
template<typename V, typename E>
int layeredLprop<V, E>::get_count()
{
    int counter;
...
template<typename V, typename E>
int layeredLprop<V, E>::binary_search(E left, E right,std::vector<V> offsets, std::vector<V> vertices, V target)
{
	E ans = 0;
...
template<typename V, typename E>
void layeredLprop<V, E>::assign_seeds_for_device (std::string seedfile, int n){
	//load seeds file.
	bool * seednodes =  new bool [n];
	memset(seednodes,false, n*sizeof(bool) );
	std::ifstream ifs(seedfile, std::ios::ate);
	ifs.seekg(0, std::ios::beg);
	std::string line;
	if(ifs.is_open()){
		std::cout << "find seed file..." <<std::endl;
	}
	...
	std::cout << "read seed file finished" <<std:: endl;
	//copy to gpu;
	cudaMemcpy(d_seednodes,seednodes,n*sizeof(bool),cudaMemcpyHostToDevice);
...
template<typename V, typename E>
void layeredLprop<V, E>::perform_lp(V n,E m,int niter)
{
	//init flag as false
	bool flag = false;
    ...
    const E huge = 10000;
    // generate seednodes then copy to gpu.
    std::string seedfile{"/home/changye/Documents/GPULP/datasets/seeds/seed_10000_1000000.txt"};

    assign_seeds_for_device(seedfile,n);
    initialize_labels_by_seeds<<<n_blocks, nthreads>>>(d_labels, G->n, d_seednodes);
...
//	// mask last few threads
    ...
    Timer t7;
    t7.start();
    first_iteration<<<n_blocks, nthreads, 0, 0>>>(d_neighbors,d_offsets,d_labels, n,d_seednodes);
    cudaDeviceSynchronize();
    t7.stop();
...
//    		gather_labels<int,int><<<m_blocks, nthreads, 0, 0>>>(d_neighbors, d_labels, d_adj_labels, m);
    	...
    	if(!opt_load||j<changeiter){
			...
			cudaDeviceSynchronize();
			if(num_huge>0){
				l_huge_update_syn2_seed_test<32,nt_huge><<<nb_huge, nt_huge, 0,0>>>( d_neighbors,d_offsets, d_adj_labels,d_vertices,d_block_v,d_block_id,d_max_count_huge,d_tables,d_seednodes);
...
//			}
			if(num_big>0){
				l_big_update_syn2_seed<32,VT, nt_big,buffer><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter, d_tables,d_seednodes);
	       }
	       if(num_medium>0){
	    	   l_medium_update_syn_seed<32,VT,nt_medium,layer><<<nb_medium1, nt_medium,0, 0>>>(big_index,medium_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter,medium,d_seednodes);
	       }
			if(num_small>0){
				l_small_update_syn_seed<32,VT,nt_small><<<nb_small, nt_small,0, 0>>>(medium_index,small_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter,d_seednodes);
			}
			if(num_tiny>0){
				l_tiny_update_syn2_seed<<<nb_tiny2, nt_tiny,0, 0>>>(d_neighbors, d_offsets, d_adj_labels,d_labels, d_counter,d_warp_v,d_warp_begin,warpnumber,d_seednodes);
			}
			...
			if(onestep == true){
				if(num_huge>0){
					l_huge_update_syn2_seed<32,nt_huge><<<nb_huge, nt_huge, 0,0>>>( d_neighbors,d_offsets, d_adj_labels,d_vertices,d_block_v,d_block_id,d_max_count_huge,d_tables,d_seednodes);
				 ...
				 if(num_big>0){
					 l_big_update_syn2_seed<32,VT, nt_big,buffer><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter, d_tables,d_if_update,d_seednodes);
				 }
				 if(num_medium>0){
					 l_medium_update_syn_seed<32,VT,nt_medium,layer><<<nb_medium1, nt_medium,0, 0>>>(big_index,medium_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter,medium,d_if_update,d_seednodes);
				 }
				if(num_small>0){
					l_small_update_syn_seed<32,VT,nt_small><<<nb_small, nt_small,0, 0>>>(medium_index,small_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter,d_if_update,d_seednodes);
				}
				if(num_tiny>0){
					l_tiny_update_syn2_seed<<<nb_tiny2, nt_tiny,0, 0>>>(d_neighbors, d_offsets, d_adj_labels,d_labels, d_counter,d_warp_v,d_warp_begin,warpnumber,d_if_update,d_seednodes);
				}
    			...
    			if(odd_even_mask){
    				...
    				if(num_huge>0){
    				 ...
    				 if(num_big>0){
    					 l_big_update_syn2_seed<32,VT, nt_big,buffer><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_adj_labels,d_sec_labels,d_vertices, d_counter, d_tables,d_if_update,d_seednodes);
    				 }
    				 if(num_medium>0){
    					 l_medium_update_syn_seed<32,VT,nt_medium,layer><<<nb_medium1, nt_medium,0, 0>>>(big_index,medium_index,d_neighbors, d_offsets, d_adj_labels,d_sec_labels,d_vertices, d_counter,medium,d_if_update,d_seednodes);
    				 }
    				 if(num_small>0){
    					 l_small_update_syn_seed<32,VT,nt_small><<<nb_small, nt_small,0, 0>>>(medium_index,small_index,d_neighbors, d_offsets, d_adj_labels,d_sec_labels,d_vertices, d_counter,d_if_update,d_seednodes);
    				 }
    				 if(num_tiny>0){
    					 l_tiny_update_syn2_seed<<<nb_tiny2, nt_tiny,0, 0>>>(d_neighbors, d_offsets, d_adj_labels,d_sec_labels, d_counter,d_warp_v,d_warp_begin,warpnumber,d_if_update,d_seednodes);
    				 }
    			}else{
    				// second iteration using d_sec_labels, output d_labels
    				if(num_huge > 0){
    					l_huge_update_syn2_seed<32,nt_huge><<<nb_huge, nt_huge, 0,0>>>( d_neighbors,d_offsets, d_sec_adj_labels,d_vertices,d_block_v,d_block_id,d_max_count_huge,d_tables,d_seednodes);
    					l_sub_huge_update_syn<32,nthreads><<<divup(num_huge,nthreads),nthreads,0,0>>>(d_max_count_huge ,  d_labels ,d_counter, huge_index,d_if_update);
    				}
    				if(num_big>0){
    					l_big_update_syn2_seed<32,VT, nt_big,buffer><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_sec_adj_labels,d_labels,d_vertices, d_counter, d_tables,d_if_update,d_seednodes);
    				}
    				if(num_medium){
    					l_medium_update_syn_seed<32,VT,nt_medium,layer><<<nb_medium1, nt_medium,0, 0>>>(big_index,medium_index,d_neighbors, d_offsets, d_sec_adj_labels,d_labels,d_vertices, d_counter,medium,d_if_update,d_seednodes);
    				}
    				if(num_small){
    					l_small_update_syn_seed<32,VT,nt_small><<<nb_small, nt_small,0, 0>>>(medium_index,small_index,d_neighbors, d_offsets, d_sec_adj_labels,d_labels,d_vertices, d_counter,d_if_update,d_seednodes);
    				}
    				if(num_tiny){
    					l_tiny_update_syn2_seed<<<nb_tiny2, nt_tiny,0, 0>>>(d_neighbors, d_offsets, d_sec_adj_labels,d_labels, d_counter,d_warp_v,d_warp_begin,warpnumber,d_if_update,d_seednodes);
    				}
...
template<typename V, typename E>
void layeredLprop<V, E>::run(int niter)
{
	this->preprocess();
