...
template<typename V, typename E>
void OTMprop<V, E>::postprocess()
{
    this->free_gmem();
}

template<typename V, typename E>
void OTMprop<V, E>::init_gmem(int n, int m)
{
...
template<typename V, typename E>
void OTMprop<V, E>::perform_lp(int n, int m,int niter)
{
    ...
    for(int j = 0;j<niter;++j){
    	otm_update_syn<32, nt><<<n_b, nt>>>(G->n,d_neighbors, d_offsets, d_labels,d_labels, d_counter);
        cudaDeviceSynchronize();
    }
    t1.stop();
    std::cout << "OTM : "<< t1.elapsed_time()<<std::endl;
}

template<typename V, typename E>
void OTMprop<V, E>::run(int niter)
{
	this->preprocess();
...
template<typename V, typename E>
void OTMprop<V, E>::testerror(int niter){
	this->preprocess();
	const int nthreads = 128;
	...
	auto prelabels = new V[G->n];
	 ...
	 for (auto i: range(niter)) {
	        ...
	        otm_update_syn<32, nt><<<n_b, nt>>>(G->n,d_neighbors, d_offsets, d_labels,d_temp_labels, d_counter);
	        cudaDeviceSynchronize();
