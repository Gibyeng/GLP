...
//	cudaMalloc(&d_big_key,      sizeof(int) * num_big);
	cudaMalloc(&d_out,      sizeof(int) * hd);
...
//    	cudaMemset(d_big_key, 0, num_big*sizeof(int));
    	...
    	if(j < 4){
    		...
    		for(int i = 0;i<num_huge;i++){
    			...
    			int* p;
    			p = d_adj_labels+G->offsets[vertices[i]];
    			int degree = G-> offsets[vertices[i]+1]- G-> offsets[vertices[i]];
    			cub::DeviceRadixSort::SortKeys(d_temp_storage,temp_storage_bytes,p,d_out,degree);
