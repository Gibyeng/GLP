...
protected:
    ...
    // Attributes
    V* labels;
    V* d_label_container; // n* label_size
    V label_size = 5;
    int *d_vertices;     // n
    int *d_neighbors;    // m
    E *d_offsets;      // n + 1
...
template<typename V, typename E>
void SLPAprop<V, E>::postprocess()
{
    this->free_gmem();
}

template<class V, class E>
void SLPAprop<V, E>::errorCheck(std::string message){
...
template<typename V, typename E>
void SLPAprop<V, E>::init_gmem(V n,E m)
{
    labels = new V[n];
    cudaMalloc(&d_label_container, sizeof(int)* n * label_size);
    ...
    int* label_container = new int [n*label_size];
    for(int i = 0;i<n*label_size;++i){
    	label_container[i] = -1;
    }
    cudaMemcpy(d_label_container, label_container, sizeof(int)*n*label_size, cudaMemcpyHostToDevice);

}

template<typename V, typename E>
void SLPAprop<V, E>::free_gmem()
{
	delete [] labels;
	cudaFree(d_label_container);
...
template<typename V, typename E>
int SLPAprop<V, E>::binary_search(E left, E right,std::vector<V> offsets, std::vector<V> vertices, V target)
{
	E ans = 0;
...
template<typename V, typename E>
double SLPAprop<V, E>::perform_lp(V n,E m,int niter)
{
...
//	// mask last few threads
    ...
    for(int j = 1;j<niter;++j){
    	...
    	t2.start();
		t5.start();
    	//store labels to container.
        storelabel<<<n_blocks, nthreads, 0, 0>>> (d_labels_write,d_label_container, n,label_size);
        //pick labels from container.
    	picklabel<<<n_blocks, nthreads, 0, 0>>> (d_labels,d_label_container, n,label_size);
	   ...
	   if(num_medium>0){
        ...
        std::cout << "in interation: " <<j<< " total time(t1): "<<t2.elapsed_time()<<", processing time(t2): "<<t3.elapsed_time()<<", candidate picking time: "<<t5.elapsed_time()<<std::endl;
    ...
    return t1+t7.elapsed_time()+t6;
}

template<typename V, typename E>
double SLPAprop<V, E>::run(int niter)
{
