...
protected:
    ...
    int *d_labels;       // n
    int *d_counter;      // 1
    int *d_temp_labels;  // n for asynchronous update
};

template<typename V, typename E>
void FAprop<V,E>::preprocess(){
...
template<typename V, typename E>
void FAprop<V, E>::postprocess()
{
    this->free_gmem();
}

template<typename V, typename E>
void FAprop<V, E>::init_gmem(int n, int m)
{
...
template<typename V, typename E>
void FAprop<V, E>::perform_lp(int n, int m,int niter)
{
    ...
    for(int j = 0;j<niter;++j){
    	fa_update_syn<nt, W><<<G->n, nt, 0>>>(d_neighbors, d_offsets, d_labels,d_labels, d_counter);
    	cudaDeviceSynchronize();
    }

    t1.stop();
    std::cout << "frequent : "<< t1.elapsed_time()<<std::endl;
}

template<typename V, typename E>
void FAprop<V, E>::run(int niter)
{
	this->preprocess();
...
template<typename V, typename E>
void FAprop<V, E>::testerror(int niter){
	this->preprocess();
	const int nthreads = 128;
	...
	auto prelabels = new V[G->n];
	 ...
	 for (auto i: range(niter)) {
	        ...
	        fa_update_syn<nt, W><<<G->n, nt, 0>>>(d_neighbors, d_offsets, d_labels,d_temp_labels, d_counter);
	        cudaDeviceSynchronize();
