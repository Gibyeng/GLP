...
template<typename V, typename E>
void CMprop<V, E>::postprocess()
{
    this->free_gmem();
}

template<typename V, typename E>
void CMprop<V, E>::init_gmem(int n, int m)
{
...
template<typename V, typename E>
void CMprop<V, E>::perform_lp(int n, int m,int niter,int v_offset)
{
    ...
    for(int j = 0;j<niter;++j){
    	 cm_update_syn<nt, W,3><<<G->n, nt>>>(d_neighbors, d_offsets, d_labels,d_labels,  d_counter);
        cudaDeviceSynchronize();
    }
    t1.stop();
    std::cout << "CM : "<< t1.elapsed_time()<<std::endl;
}

template<typename V, typename E>
void CMprop<V, E>::run(int niter)
{
	this->preprocess();
...
template<typename V, typename E>
void CMprop<V, E>::testerror(int niter){
	this->preprocess();
	const int nthreads = 128;
	...
	auto prelabels = new V[G->n];
	 ...
	 for (auto i: range(niter)) {
	        ...
	        cm_update_syn<nt, W,3><<<G->n, nt>>>(d_neighbors, d_offsets, d_labels,d_temp_labels,  d_counter);
	        cudaDeviceSynchronize();
	        ...
	        std::cout << "CPU changed label:"<< nupdates<< std::endl;
	        std::cout << "GPU changed label:"<< get_count()<< std::endl;
//	        std::cout << "GPU ";
