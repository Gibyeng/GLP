...
template<typename V, typename E>
void PushLprop<V, E>::postprocess()
{
    this->free_gmem();
}

template<typename V, typename E>
void PushLprop<V, E>::init_gmem(V n,E m)
{
...
void PushLprop<V, E>::free_gmem()
{
    ...
    cudaFree(d_r_offsets);
}

// Return the number of labels updated
template<typename V, typename E>
int PushLprop<V, E>::get_count()
{
    int counter;
...
template<typename V, typename E>
int PushLprop<V, E>::binary_search(E left, E right,std::vector<V> offsets, std::vector<V> vertices, V target)
{
	E ans = 0;
...
template<typename V, typename E>
void PushLprop<V, E>::perform_lp(V n,E m,int niter)
{
	...
	int *r_neighbors = new int [m];
	int *r_offsets = new int [n+1];
	cudaMemcpy(r_neighbors,d_neighborval_out,m*sizeof(int),cudaMemcpyDeviceToHost);
	memset(r_offsets,0,(n+1)*sizeof(int));
	for (int i = 0; i < m ; i++){
		int now = r_neighbors[i];
		int pre = now;
		if(i!=1){
			pre = r_neighbors[i-1];
		}
		if(pre != now){
			for(int j = pre+1;j<now+1;j++){
				r_offsets[j] = i;
			}
		}
	}
	r_offsets[n] = m-1;
	cudaMemcpy(d_r_offsets,r_offsets, sizeof(int)* (n+1),cudaMemcpyHostToDevice);
//	printf("\n");
    Timer t0;
    t0.start();
...
//    		gather_labels<int,int,VT_m><<<m_blocks_div, nthreads, 0, 0>>>(d_neighbors, d_labels, d_adj_labels, m);
    		push_gather_labels<int,int><<<n, 64, 0, 0>>>(d_neighborkey_out,d_r_offsets,d_labels, d_adj_labels, n,d_if_update);
    	}
    	cudaDeviceSynchronize();
    	t5.stop();
		...
		if(j<changeiter){
			...
			if(num_big > 0){
				l_big_update_syn2<32,VT, nt_big,800><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter, d_tables);
			}
			if(num_medium >0){
			...
			if(num_big > 0){
				l_big_update_syn2<32,VT, nt_big,400><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter, d_tables,d_if_update);
			}
			if(num_medium >0){
...
template<typename V, typename E>
void PushLprop<V, E>::run(int niter)
{
	this->preprocess();
