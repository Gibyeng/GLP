...
template<typename V, typename E>
void Ml3prop<V, E>::postprocess()
{
    this->free_gmem();
}

template<typename V, typename E>
void Ml3prop<V, E>::init_gmem(V n,E m)
{
...
template<typename V, typename E>
int Ml3prop<V, E>::binary_search(E left, E right,std::vector<V> offsets, std::vector<V> vertices, V target)
{
	E ans = 0;
...
template<typename V, typename E>
void Ml3prop<V, E>::perform_lp(V n,E m,int niter)
{
	//init flag as false
	bool flag = false;
...
//	printf("\n");
	...
	int nb_huge = 0;
	// assign memory for deal with huge nodes
	if(num_huge>0){
		for(int i = 0;i<num_huge;i++){
			int degree = G->offsets[i+1]-G->offsets[i];
			nb_huge += divup(degree,nt_huge);
		}
//		 block -> node id
		int*  block_v = new int [nb_huge];
		// the block id handle the same vertex
		int*  block_id = new int [nb_huge];
		int begin = 0;
		for(int i = 0;i<num_huge;i++){
			int degree = G->offsets[i+1]-G->offsets[i];
			int end = divup(degree,nt_huge) +begin;
		...
		std::cout<< " block : "<< nb_huge << std::endl;
		 cudaMalloc(&d_block_v,      sizeof(int) * nb_huge);
		 cudaMalloc(&d_block_id,      sizeof(int) * nb_huge);
		 cudaMemcpy(d_block_v,block_v,sizeof(int)*nb_huge, cudaMemcpyHostToDevice);
		 cudaMemcpy(d_block_id,block_id,sizeof(int)*nb_huge, cudaMemcpyHostToDevice);
		 cudaMalloc(&d_max_score, sizeof(int)*num_huge);
		 cudaMemset(d_max_score, 0, sizeof(int)*num_huge);
		 cudaMalloc(&d_cm, sizeof(SharedCM<200,3>)*num_huge);
		 cudaMemset(d_cm, 0, sizeof(SharedCM<200,3>)*num_huge);
	}
...
//	        ml_huge_update_syn3_cmp<32,nt_huge><<<nb_huge, nt_huge, 0,0>>>(0,huge_index, d_neighbors,d_offsets, d_adj_labels,d_labels,d_vertices,d_block_v,d_block_id,d_max_score, d_counter, d_gcounter, d_gcounter_temp,delta,d_cm);
			ml_big_update_syn3<32,VT, nt_big,32,256><<<(num_big+VT-1)/VT, nt_big, 0,0>>>(huge_index,big_index,d_neighbors, d_offsets, d_adj_labels,d_labels,d_vertices, d_counter,d_gcounter,d_gcounter_temp,delta);
...
template<typename V, typename E>
void Ml3prop<V, E>::run(int niter)
{
	this->preprocess();
...
template<typename V, typename E>
void Ml3prop<V, E>::testerror(int niter){
